...
def sjf(processos, quantum, sobrecarga):
    if processos == 0:
        ...
        for index, processo in enumerate(processos):
            if index == 0:
                print("---------------------- INICIO SJF ----------------------")
                print(f"proc {index} " + (' ' * (output_size - len(str(index)))) + (
                            int(processo["tempo_de_execucao"]) * "■"))
            else:
                tempo_de_espera += int(processos[index - 1]["tempo_de_execucao"])
                print(f"proc {index} " + (' ' * (output_size - len(str(index)))) + ("□" * tempo_de_espera) + (
                            int(processo["tempo_de_execucao"]) * "■"))
        print("-----------------------------------------------\n")
        return menu.main_menu(processos, quantum, sobrecarga)
